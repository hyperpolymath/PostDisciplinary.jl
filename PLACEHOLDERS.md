...
All placeholders in this template follow the `PostDisciplinary.jl` pattern.
...
sed -i 's/PostDisciplinary.Jl/my-project/g' $(grep -rl 'PostDisciplinary.Jl' .)
sed -i 's/PostDisciplinary.jl/MY_PROJECT/g' $(grep -rl 'PostDisciplinary.jl' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `PostDisciplinary.Jl` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.guix, devcontainer.json |
| `PostDisciplinary.jl` | One-line description | `A tool for X` | flake.guix |
| `PostDisciplinary.jl` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/abi/*.idr, ffi/zig/*.zig |
| `{{project}}` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, ffi/zig/*.zig |
| `PostDisciplinary.jl` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `PostDisciplinary.jl` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
